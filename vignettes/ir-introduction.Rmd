...
ir relies on [ggplot](https://cran.r-project.org/web/packages/ggplot2/index.html). This makes it possible to modify the plot with the functions from ggplot2. For example, we could color spectra according to the sample class:
...
Baseline correction with a rubberband algorithm (see the `spc.rubberband` function in the [hyperspec](https://cran.r-project.org/web/packages/hyperSpec/index.html) package):
...
The warning just says that the spectrum's wavenumber values did not exactly match the desired value and therefore the nearest value available was selected. To disable this warning, you can interpolate the spectrum appropriately (see [below](Interpolating)).

## Smoothing

Smoothing of spectra with the Savitzky-Golay algorithm (see the `sgolayfilt()` function from the [signal](https://cran.r-project.org/web/packages/signal/index.html) package for details):
...
This is not easy to see from the plot, but the warning shown [above](Normalization) during normalization now does not appear:
...
With ir, it is very easy to build complex reprocessing workflows by "piping" (using [magrittr's](https://cran.r-project.org/web/packages/magrittr/index.html) pipe (`%>%`) operator) together different preprocessing steps:
